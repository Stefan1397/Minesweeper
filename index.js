...
function clickCell() {
    ...
    numberOfNeighboringBombs(lineIndex, columnIndex);
    if (bombsPosition.includes(this.id)) {
        gameOver = true;
        revealBombs();
    }
...
function numberOfNeighboringBombs(lineIndex, columnIndex) {
    ...
    for (let line = lineIndex - 1; line <= lineIndex + 1; ++line) {
        for (let column = columnIndex - 1; column <= columnIndex + 1; ++column) {
            numberBombs += containsBomb(line, column);
    ...
    if (numberBombs > 0) {
        ...
        for (let line = lineIndex - 1; line <= lineIndex + 1; ++line) {
            for (let column = columnIndex - 1; column <= columnIndex + 1; ++column) {
                numberOfNeighboringBombs(line, column);
            }
        }
    }
}

function containsBomb(line, column) {
